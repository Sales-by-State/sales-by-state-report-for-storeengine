...
class Controller extends \WP_REST_Controller {
	...
	private function report_args() {
		return array(
			'year'     => array(
				'description' => __( 'Calendar year to report on.', 'sales-by-state-report-for-storeengine' ),
				'type'        => 'integer',
				'default'     => Filters::default_year(),
			),
			'country'  => array(
				'description'       => __( 'Two-letter country code.', 'sales-by-state-report-for-storeengine' ),
				'type'              => 'string',
				'default'           => Filters::default_country(),
				'sanitize_callback' => 'sanitize_text_field',
			),
			'statuses' => array(
				'description'       => __( 'Comma-separated order statuses to include.', 'sales-by-state-report-for-storeengine' ),
				'type'              => 'string',
	...
	public function get_report( $request ) {
		...
		if ( ! Schema::table_exists() ) {
			return new \WP_Error(
				'sbsse_no_table',
				__( 'The report table could not be created. Check the database user has permission to create tables.', 'sales-by-state-report-for-storeengine' ),
				array( 'status' => 500 )
			);
		}
	...
	public function get_item_schema() {
		...
		$row = array(
			'state'      => array(
				'description' => __( 'State code.', 'sales-by-state-report-for-storeengine' ),
				'type'        => 'string',
				'context'     => array( 'view' ),
				'readonly'    => true,
			),
			...
			'state_name' => array(
				'description' => __( 'State name.', 'sales-by-state-report-for-storeengine' ),
				'type'        => 'string',
				'context'     => array( 'view' ),
				'readonly'    => true,
			),
		...
		$this->schema = array(
			...
			'properties' => array(
				'rows'   => array(
					'description' => __( 'One entry per state.', 'sales-by-state-report-for-storeengine' ),
					'type'        => 'array',
					'context'     => array( 'view' ),
					'readonly'    => true,
					'items'       => array(
				...
				'totals' => array(
					'description' => __( 'Totals across every state.', 'sales-by-state-report-for-storeengine' ),
					'type'        => 'object',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'total'  => array(
					'description' => __( 'Number of states in the result.', 'sales-by-state-report-for-storeengine' ),
					'type'        => 'integer',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
